...
function Checkout() {
  ...
  // Fix: Change the function to accept a single object argument
  const openRazorpay = ({ orderId, amount, key, orderDocId }) => {
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        handler: async function (response) {
            try {
                await axios.post(
                  `${config.backendUrl}/api/orders/verify-payment`,
                  {
                    orderDocId: orderDocId, // Fix: Use the correctly passed variable
                ...
                setError('Payment successful, but order could not be finalized. Please contact support.');
            }
        },
        prefill: {
          name: address.name,
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { orderId, key, amount, orderDocId } = createOrderRes.data;

      // Fix: Pass the data as a single object
      await openRazorpay({ orderId, key, amount, orderDocId });
